...
namespace RDS.ViewModels.Common
{
	...
	{
        public static readonly DependencyProperty IsEnabledProperty = DependencyProperty.RegisterAttached(nameof(UIElement.IsEnabled), typeof(bool), typeof(SingletonWindow), new FrameworkPropertyMetadata(OnIsEnabledChanged));
		...
		static void HandleRunningInstance(System.Diagnostics.Process instance)
		{
			if (instance.MainWindowHandle != IntPtr.Zero)
			{
				for (int i = 0; i < 2; i++)
				{
					FlashWindow(instance.MainWindowHandle, 0);
				}
				SetForegroundWindow(instance.MainWindowHandle);
				ShowWindowAsync(instance.MainWindowHandle, SingletonWindow.WS_SHOWNORMAL);
			}
		}
